...
class Nova_Ajax_Search {
	...
	public function nova_get_search_results() {
		...
		if ( !empty( $products ) ) {
			...
			if (count($products) == 6) {
				$output['suggestions'].= '<a class="view-all" href="#">' . esc_html__('View All', 'juliette') . '</a>';
			}
			wp_reset_postdata();

		} else {
			$output['suggestions'] =  '<div class="search-no-suggestions"><span class="search-st">' . esc_html__('No results', 'juliette') . '</span></div>';
		}
		if( Nova_OP::getOption('header_search_style') == 'default' ) {
			$output['suggestions'] = '<span class="product-search-heading">' . esc_html__("Search results", "juliette") . '</span><div class="grid-x grid-padding-x grid-padding-y small-up-1">' . $output['suggestions'] .'</div>';
		}else {
			$output['suggestions'] = '<span class="product-search-heading">' . esc_html__("Search results", "juliette") . '</span><div class="grid-x grid-padding-x grid-padding-y small-up-1 medium-up-2 large-up-3">' . $output['suggestions'] .'</div>';
		}
