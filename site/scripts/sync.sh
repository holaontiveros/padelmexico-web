...
DEVSITE="https://padelmexico.lndo.site"

PRODDIR="padelmexico@ssh.dean:/home/padelmexico/htdocs/padelmexico.com.mx/shared/uploads/"
PRODSITE="https://padelmexico.com.mx"

STAGDIR="web@ssh.dean:/home/padelmexico/htdocs/padelmexico.com.mx/shared/uploads/"
STAGSITE="https://staging.padelmexico.com.mx"
...
if [ $# != 2 ]
then
  echo "Usage: $0 [[--skip-db] [--skip-assets] [--local] [--lando]] [ENV_FROM] [ENV_TO]"
exit;
fi
...
case "$1-$2" in
  ...
  *) echo "usage: $0 [[--skip-db] [--skip-assets] [--local] [--lando]] production development | staging development | development staging | development production | staging production | production staging" && exit 1 ;;
esac
...
if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
  ...
  echo "Syncing database..."
    ...
    if [[ "$LOCAL" = true && $TO == "development" ]]; then
      ...
      wp "@$TO" db export --default-character-set=utf8mb4 &&
      wp "@$TO" db reset --yes &&
      wp "@$FROM" db export --default-character-set=utf8mb4 - | sed 's/utf8mb4_0900_ai_ci/utf8mb4_general_ci/g' | wp "@$TO" db import - &&
      wp "@$TO" search-replace "$FROMSITE" "$TOSITE" --all-tables-with-prefix
    fi
  fi
