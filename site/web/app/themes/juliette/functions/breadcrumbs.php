...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'juliette' ),
			'archive'   => esc_html__( 'Archives', 'juliette' ),
			'blog'      => esc_html__( 'Blog', 'juliette' ),
			'search'    => esc_html__( 'Search results for', 'juliette' ),
			'not_found' => esc_html__( 'Not Found', 'juliette' ),
			'author'    => esc_html__( 'Author Archives:', 'juliette' ),
			'day'       => esc_html__( 'Daily Archives:', 'juliette' ),
			'month'     => esc_html__( 'Monthly Archives:', 'juliette' ),
			'year'      => esc_html__( 'Yearly Archives:', 'juliette' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'juliette' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'juliette' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'juliette' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
