...
?>
		<div class="off-canvas-wrapper">
				...
				<div class="nova-offcanvas sizeguide-canvas site-canvas-menu off-canvas position-right" id="SizeGuide" data-off-canvas data-transition="overlap">
					<h2 class="title"><?php echo esc_html__( 'Size Guidelines', 'juliette' );?></h2>
				...
				<?php if ( NOVA_WOOCOMMERCE_IS_ACTIVE ) : ?>
					<?php if ( Nova_OP::getOption('header_cart') == 1 ) : ?>
						<div class="nova-offcanvas minicart-canvas site-canvas-menu off-canvas position-right" id="MiniCartCanvas" data-off-canvas data-transition="overlap">
							<h2 class="title"><?php echo esc_html__( 'Shopping Cart', 'juliette' );?> (<span class="nova_js_count_bag_item_canvas"><?php echo esc_html(WC()->cart->get_cart_contents_count()); ?></span>)</h2>
					...
					<?php if ( is_product() ) : ?>
						...
						<?php if ( ! empty( $tabs ) ) : ?>
							<?php foreach ( $tabs as $key => $tab ) : ?>
								<div class="nova-offcanvas sizeguide-canvas site-canvas-menu off-canvas position-right" id="product_data_tabs_<?php echo esc_attr( $key ); ?>" data-off-canvas data-transition="overlap">
