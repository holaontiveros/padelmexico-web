<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<div class="nova-blog-posts post-display-flex">
			...
			if ( has_post_format( 'quote' ) && NOVA_RWMB_IS_ACTIVE):
				$quote_content = rwmb_meta('juliette_post_quote_content');
				$quote_author = rwmb_meta('juliette_post_quote_author');
				$quote_background = rwmb_meta('juliette_post_quote_bg_color');
				$quote_color = rwmb_meta('juliette_post_quote_text_color');
			...
			<div class="nova-blog-posts__content<?php if(has_post_thumbnail()): ?> has-thumbnail<?php endif?>">
					...
					if( $excerpt_limit ) {
						if($excerpt_length > 0){
								echo sprintf(
										...
										juliette_excerpt(intval( $excerpt_length ))
								);
						}
					}else {
						echo sprintf(
				...
				<div class="entry-meta">
					...
					<?php echo esc_html__( 'by', 'juliette' );?>
