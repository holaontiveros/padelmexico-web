...
class Portfolio extends NOVA_Widget_Base {
    ...
    protected function _register_controls() {
                ...
                'fields_options' => [
                    'post_type' => [
                        ...
                        'options' => [
                            ...
                            'nova_portfolio' => __( 'Latest Portfolio', 'novaworks' ),
                    ...
                    'exclude_term_ids' => [
                        ...
                        'object_type' => 'nova_portfolio_category',
                    ],
                    'include' => [
                        'options' => [
                            'terms' => __( 'Portfolio Category', 'novaworks' ),
        ...
        $portfolio_categories = get_terms( 'nova_portfolio_category' );
    ...
    public function generate_carousel_setting_json(){
        ...
        $tablet_col = absint( isset($settings['columns_tablet']) ? $settings['columns_tablet'] : 0 );
        $tabletportrait_col = absint( isset($settings['columns_tabletportrait']) ? $settings['columns_tabletportrait'] : 0 );
        $mobile_col = absint( isset($settings['columns_mobile']) ? $settings['columns_mobile'] : 0 );
    ...
    public function render_filters() {
        ...
        foreach ( $category_list_id as $cat_slug ){
            $tax_obj = get_term_by('slug', $cat_slug, 'nova_portfolio_category');
        ...
        foreach ( $category_list as $slug => $category_name ) {
            ...
            $html .= sprintf( '%3$s<div class="isotope__filter-item novaworks-portfolio__filter-item" data-filter="%1$s"><span>%2$s</span></div>', 'nova_portfolio_category-' . $slug, $category_name, $separator_html );
        }

        echo sprintf( '<div class="isotope__filter novaworks-portfolio__filter js-el" data-nova_component="MasonryFilter" data-isotope_container="%2$s"><div class="isotope__filter-list novaworks-portfolio__filter-list">%1$s</div></div>', $html , '.elementor-element-'. $this->get_id().' .la-isotope-container' );
