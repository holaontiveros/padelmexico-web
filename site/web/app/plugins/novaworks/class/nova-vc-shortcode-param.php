...
class Nova_Shortcodes_Param{
    ...
    public function columnResponsiveCallBack($settings, $value){
        ...
        $html .= '</div>';
        $html .= '<script type="text/javascript">jQuery("#'.$uid.'").trigger("vc_param.nova_columns")</script>';
        return $html;
    }
    ...
    public function hotspot_image_preview( $settings, $value ){
        ...
        return '<div id="nova_image_with_hotspots_preview"><input name="' . esc_attr( $settings['param_name'] ) . '" type="hidden" class="wpb_vc_param_value ' . esc_attr( $settings['param_name'] ) . '" value="'.esc_attr($value).'" /> '.$image_output. '</div>';
    }
